...
//	xil_printf("DIE_MAP_ADDR : %8x\r\n", DIE_MAP_ADDR);
	...
	for(i=0 ; i<DIE_NUM ; i++)
	{
		...
		dieBlock->dieEntry[i].numFreeBlocks = BLOCK_NUM_PER_DIE;
	}
...
int FindFreePage(u32 dieNo)
{
	...
	{
		dieBlock->dieEntry[dieNO].numFreeBlocks--;
		// If ratio of free blocks is less than threshold, do garbage collection for 1 block.
		if (dieBlock->dieEntry[dieNo].numFreeBlocks < (BLOCK_NUM_PER_DIE*GC_THRESHOLD)/100))
			dieBlock->dieEntry[dieNo].currentBlock = GarbageCollection(dieNo);

		// Move to next block (for wear leveling, we do not use the block that has just been freed by GC)
