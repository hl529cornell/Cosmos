...
//--------------------------------------------------
// FTL internal function prototype for Wear Leveling
//--------------------------------------------------

static void   write_page_WL(UINT32 const lpn, UINT32 const sect_offset, UINT32 const num_sectors);
static UINT32 assign_new_write_vpn_WL(UINT32 const bank) ;
static void   garbage_collection_WL(UINT32 const bank);
static void   tc_write_WL_test (const UINT32 io_num, const UINT32 sector_size) ;
void ftl_write_WL(UINT32 const lba, UINT32 const num_sectors) ;
...
void ftl_open(void)
{
  ...
  if (TRUE)
    {
      uart_print("jmukund: Begin Formatting" ) ; 
      uart_print("do format");
      format();
      uart_print("end format");
      uart_print("jmukund: End Formatting" ) ; 
      
      uart_printf ( "jmukund: Num Banks: %u", NUM_BANKS ) ;
      uart_printf ( "jmukund: Sectors Per Page: %u", SECTORS_PER_PAGE ) ;
      uart_printf ( "jmukund: Pages Per Block(PAGES_PER_BLK): %u", PAGES_PER_BLK ) ;
      uart_printf ( "jmukund: Pages Per Virtual Block(PAGES_PER_VBLK): %u", PAGES_PER_VBLK ) ;
      uart_printf ( "jmukund: Virtual Blocks Per Bank (VBLKS_PER_BANK): %u", VBLKS_PER_BANK ) ;
      
      uart_printf ( "jmukund: Num Logical Pages (NUM_LPAGES): %u", NUM_LPAGES ) ;
      uart_printf ( "jmukund: Num Virtual Pages : %u", VBLKS_PER_BANK * PAGES_PER_BLK ) ; 
      
      uart_printf ( "jmukund: Max Valid Page Count of vblock (VC_MAX): %u", VC_MAX ) ;
  ...
  flash_clear_irq();
    ...
    SETREG(INTR_MASK, FIRQ_CORRECTED | FIRQ_DATA_CORRUPT | FIRQ_BADBLK_L | FIRQ_BADBLK_H);
    SETREG(FCONF_PAUSE, FIRQ_DATA_CORRUPT | FIRQ_BADBLK_L | FIRQ_BADBLK_H);
    
    uart_print("jmukund: Start Timer") ; 
    ptimer_start();
    
    //ftl_test_write ( 100, 8 ) ; 
    //ftl_test_write ( 200, 8 ) ; 

    ptimer_stop_and_uart_print();
    uart_print("jmukund: End   Timer") ; 
...
#define STARTING_VBLK    66 
#define ENDING_VBLK      68 
#define DIFF_VBLK        2
#define CHOSEN_BANK      4

#define init_new_write_vpn(bank, vpn)  (g_misc_meta[bank].cur_write_vpn = vpn)

#define is_full_all_blks_userlimit(bank)  (g_misc_meta[bank].free_blk_cnt == 1 ) 
#define init_free_blk_cnt_WL(bank, threshold) (g_misc_meta[bank].free_blk_cnt = threshold )
#define inc_full_blk_cnt_WL(bank)  (g_misc_meta[bank].free_blk_cnt --)
#define dec_full_blk_cnt_WL(bank)  (g_misc_meta[bank].free_blk_cnt ++)

static void init_ftl_WL_test ( ) {
  UINT32 bank_num = CHOSEN_BANK ;;
  UINT32 vblock_num = STARTING_VBLK ; // 66
  UINT32 gc_vblock_num = ENDING_VBLK ; // 68
  UINT32 free_blk_threshold = 2 ; // ENDING_VBLK - STARTING_VBLK ; // 2
  UINT32 i = 0 ;

  // erase blocks from 66 to 76 in bank 4
  // TO DO

  for ( i = STARTING_VBLK ; i <= ENDING_VBLK ; i ++ ) {
    nand_block_erase(bank_num, i);
    set_vcount(bank_num, i, VC_MAX_ERASE) ; 
    uart_printf ( "Block: %d in Bank: %d successfully erased\tnum valid pages: %d" , i, bank_num, get_vcount(bank_num, i ) ) ;
  }

  init_new_write_vpn ( bank_num,  (vblock_num * PAGES_PER_BLK)-1 ) ;
  set_gc_vblock ( bank_num, gc_vblock_num ) ;
  init_free_blk_cnt_WL ( bank_num, free_blk_threshold ) ;

}


void ftl_WL_test(void) {
  uart_print("start ftl WL test...");

  // initialize the lba to pba table 
  init_ftl_WL_test () ; 
  
  uart_printf("Debug: Get Current Write VPN %u", get_cur_write_vpn(CHOSEN_BANK) ) ; 
  uart_printf("Debug: Get GC Vblock: %u", get_gc_vblock(CHOSEN_BANK) ) ; 
  uart_printf("Debug: Get Init Free Block Count: %u", g_misc_meta[CHOSEN_BANK].free_blk_cnt ) ; 
  
  tc_write_WL_test( 130, SECTORS_PER_PAGE ) ; 
...
static void tc_write_WL_test (const UINT32 io_num, const UINT32 sector_size)
{
	...
	uart_printf("\nIteration Num: %d\tLBA: %u data: %u  num_sectors: %u", i, lba, data, num_sectors);
	
	/*             ptimer_start(); */
	ftl_write_WL(lba, num_sectors);
	/*             ptimer_stop_and_uart_print(); */
	...
	UINT32 sample ;
	for (j = 0; j < num_sectors; j++) {
	  sample = read_dram_32(rd_buf_addr);
	  ...
	  if (sample != r_data) {
	    ...
	    //uart_printf("ftl test passed...io#: %d, %d", lba, num_sectors);
	    //uart_printf("sample data %d should be %d", sample, r_data);
	  //}
	...
	if ( !write_fail ) {
	  //uart_printf("ftl test passed: Successfully written to page num: %d\tblock num: %d", get_cur_write_vpn(CHOSEN_BANK), get_cur_write_vpn(CHOSEN_BANK)/PAGES_PER_BLK ) ; 
	  //uart_printf("sample data %d should be %d", sample, r_data-1);
	}
...
void ftl_write_WL(UINT32 const lba, UINT32 const num_sectors) {
  ...
  while (remain_sects != 0) {
    ...
    // single page write individually
    write_page_WL(lpn, sect_offset, num_sectors_to_write);
...
static void write_page_WL(UINT32 const lpn, UINT32 const sect_offset, UINT32 const num_sectors) {
  ...
  uart_printf ( "DEBUG: lpn: %u\tsect_offset: %u\tnum_sectors: %u\tbank: %u\t", lpn, sect_offset, num_sectors, bank ) ;

  new_vpn  = assign_new_write_vpn_WL(bank);
  old_vpn  = get_vpn(lpn);
  ...
  if (old_vpn != NULL) {
    ...
    uart_printf ( "Old data exists: old_vpn: %u\told_block: %u\tvalid pages in old block: %u\told page number: %u", old_vpn, vblock, get_vcount(bank, vblock),page_num ) ;
    ...
    uart_printf ( "Old data does not exist" ) ;
  }
  ...
  uart_printf("Debug: New data has been written\tCurrent Bank Num: %u\tNew_VPN: %u\tNew Block: %u  vcount: %d\tNew Page Number: %u", bank, new_vpn, vblock, get_vcount(bank,vblock),page_num ) ; 
...
static UINT32 assign_new_write_vpn_WL(UINT32 const bank) {
  ...
  if ((write_vpn % PAGES_PER_BLK) == (PAGES_PER_BLK - 2)) {
    ...
    mem_copy(FTL_BUF(bank), g_misc_meta[bank].lpn_list_of_cur_vblock, sizeof(UINT32) * PAGES_PER_BLK);
    // fix minor bug
    nand_page_ptprogram(bank, vblock, PAGES_PER_BLK - 1, 0, ((sizeof(UINT32) * PAGES_PER_BLK + BYTES_PER_SECTOR - 1 ) / BYTES_PER_SECTOR), FTL_BUF(bank));
    
    mem_set_sram(g_misc_meta[bank].lpn_list_of_cur_vblock, 0x00000000, sizeof(UINT32) * PAGES_PER_BLK);
    
    inc_full_blk_cnt_WL(bank);
    
    // do garbage collection if necessary
    //if (is_full_all_blks(bank)) {

    if (is_full_all_blks_userlimit(bank)) {
      uart_printf ( "\nAll blocks full in SSD\tNeed to do Garbage Collection" ) ;
      garbage_collection_WL(bank);
      return get_cur_write_vpn(bank);
    }

    do {       
      vblock ++ ;
      if ( vblock > ENDING_VBLK ) 
	vblock = STARTING_VBLK ;
      ASSERT(vblock != VBLKS_PER_BANK);
    }while (get_vcount(bank, vblock) == VC_MAX);
  }
...
static UINT32 get_vt_vblock_WL(UINT32 const bank) {
    ASSERT(bank < NUM_BANKS);

    UINT32 vblock, i, min_vcount, vcount ;

    // search the block which has mininum valid pages
    min_vcount = get_vcount(bank, STARTING_VBLK) ; 
    vblock = STARTING_VBLK ;

    uart_printf ( "Starting min: %u\tvblock: %u: ", min_vcount , vblock ) ;

    for ( i = STARTING_VBLK+1 ; i <= ENDING_VBLK ; i ++ ) {
      vcount  = get_vcount(bank, i ) ; 
      uart_printf ( "Next dram read: %u\tvblock: %u: ", vcount , i ) ;

      if ( vcount != VC_MAX_ERASE && vcount < min_vcount ) {
	min_vcount = vcount ;
	vblock = i ;
      }      
    ...
    uart_printf ( "Final min: %u\tvblock: %u: ", min_vcount , vblock ) ;


    //    vblock = mem_search_min_max ( (VCOUNT_ADDR + (((bank * VBLKS_PER_BANK) + STARTING_VBLK) * sizeof(UINT16))), 
    //				  sizeof(UINT16),
    //				  STARTING_VBLK+4,  
    //				  MU_CMD_SEARCH_MIN_DRAM);
    

    //uart_printf("Chosen Victim Block with Least number of Valid Pages: %u\tVCOUNT_ADDR: %u\tBlock StartAddr: %u\tStarting Address: %u", vblock, 
    //		VCOUNT_ADDR, (((bank * VBLKS_PER_BANK) + STARTING_VBLK) * sizeof(UINT16)), (VCOUNT_ADDR + (((bank * VBLKS_PER_BANK) + STARTING_VBLK) * sizeof(UINT16))) ) ; 
...
static void garbage_collection_WL(UINT32 const bank)
{
    ...
    vt_vblock = get_vt_vblock_WL(bank);   // get victim block
    vcount    = get_vcount(bank, vt_vblock);
    gc_vblock = get_gc_vblock(bank);
    free_vpn  = gc_vblock * PAGES_PER_BLK;

    uart_printf("garbage_collection bank %d, vblock %d\tGC_Vblock: %u\tFree VPN: %u",bank, vt_vblock, gc_vblock, free_vpn); 
	...
	//uart_printf ( "\nsrc_page: %d\tget_vpn: %d\t (vt_vblock*PAGES_PER_BLK) + src_page: %u" , src_lpn, get_vpn(src_lpn) , (vt_vblock*PAGES_PER_BLK) + src_page ) ;

        // determine whether the page is valid or not
        if (get_vpn(src_lpn) !=  ((vt_vblock * PAGES_PER_BLK) + src_page)) {
	  // invalid page
	  //uart_printf ( "\nInvalid Page. Continue: src_lpn: %u\tget_vpn(src_lpn): %u\tvt_vblock*PAGES_PER_BLOCK+src_page: %u", src_lpn, get_vpn(src_lpn), ((vt_vblock * PAGES_PER_BLK) + src_page) ) ;
	  continue;
        }
	uart_printf ( "Src page %d is valid", src_page ) ;
	...
	uart_printf ( "\nCopy Valid page from source block %d page %d to destination block %d page %d", vt_vblock, src_page, free_vpn/PAGES_PER_BLK, free_vpn%PAGES_PER_BLK ) ;
        // update metadata
        set_vpn(src_lpn, free_vpn);
        set_lpn(bank, (free_vpn % PAGES_PER_BLK), src_lpn);

	uart_printf ( "GC: get_vpn(%d, %d): %d", src_lpn, free_vpn, get_vpn(src_lpn) ) ; 
	uart_printf ( "GC: get_lpn(%d, %d, %d): %d", bank, free_vpn%PAGES_PER_BLK, src_lpn, get_lpn(bank, free_vpn%PAGES_PER_BLK) ) ; 
    ...
    nand_block_erase(bank, vt_vblock);
    uart_printf ( "Garbage Collection Called\tBlock: %d successfully erased\tNew Victim block: %d" , vt_vblock, get_gc_vblock(bank)  ) ;      
    ASSERT((free_vpn % PAGES_PER_BLK) < (PAGES_PER_BLK - 2));
    ASSERT((free_vpn % PAGES_PER_BLK == vcount));
...
/*     uart_printf("gc page count : %d", vcount); */
    ...
    // 4. update metadata
    set_vcount(bank, vt_vblock, VC_MAX_ERASE);
    set_vcount(bank, gc_vblock, vcount);
    set_new_write_vpn(bank, free_vpn); // set a free page for new write
    set_gc_vblock(bank, vt_vblock); // next free block (reserve for GC)
    dec_full_blk_cnt(bank); // decrease full block count
    /* uart_print("garbage_collection end"); */
    uart_printf ( "Next GC Block: %d", vt_vblock ) ; 
}
